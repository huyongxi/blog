...
SPDLOG_INLINE void set_formatter(std::unique_ptr<blog::spdlog::formatter> formatter) {
    details::registry::instance().set_formatter(std::move(formatter));
}

SPDLOG_INLINE void set_pattern(std::string pattern, pattern_time_type time_type) {
    set_formatter(
        std::unique_ptr<blog::spdlog::formatter>(new pattern_formatter(std::move(pattern), time_type)));
}
...
SPDLOG_INLINE std::shared_ptr<blog::spdlog::logger> default_logger() {
    return details::registry::instance().default_logger();
}

SPDLOG_INLINE blog::spdlog::logger *default_logger_raw() {
    return details::registry::instance().get_default_raw();
}

SPDLOG_INLINE void set_default_logger(std::shared_ptr<blog::spdlog::logger> default_logger) {
